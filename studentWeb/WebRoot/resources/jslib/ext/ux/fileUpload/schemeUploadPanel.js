...
var keel={};
var listName="";
keel.schemeUploadPanel = function(cfg){
	this.width = 510;
	this.height = 200;
	Ext.apply(this,cfg);	
	...
	this.setting = {
		...
		swfupload_loaded_handler : function(){
			initUpload(this.customSettings.scope_handler);
	...
	keel.schemeUploadPanel.superclass.constructor.call(this,{				
		tbar : [
			{text:'添加文件',ref:'../addBtn',iconCls:'add-icon'},'-',
			{text:'上传',ref:'../uploadBtn',iconCls:'yes-icon',handler:this.startUpload,scope:this}
//			{text:'停止上传',ref:'../stopBtn',iconCls:'no-icon',handler:this.stopUpload,scope:this,disabled:true},'-'
//			{text:'删除所有',ref:'../deleteBtn',iconCls:'del-icon',handler:this.deleteAll,scope:this},'-'
		],
		layout : 'fit',
		items : [this.gp],
		listeners : {
...
Ext.extend(keel.schemeUploadPanel,Ext.Panel,{
...
//	   		fileName : file.name,
	...
	formatFileSize : function(_v, celmeta, record) {//文件大小
	...
	formatProgressBar : function(v){//进度条属性列表的显示
	...
	onUploadComplete : function(file) { //上传成功后，改变附件的状态
	...
	onFileQueued : function(file) { //加载上传附件信息，并判断是否存在。
		var me = this.customSettings.scope_handler;
		if(listName.indexOf(file.name)>=0){
				var file_id = file.id;
				var ds = me.gp.store;
				for(var i=0;i<ds.getCount();i++){
					var rec =ds.getAt(i);
				...
				Ext.Msg.alert('提示','['+file.name+'] 文件已经存在！');
				return;
		}
		listName += file.name+",";
	...
	onUploadSuccess : function(file, serverData) {//上传附件成功函数
		var me = this.customSettings.scope_handler;
		var ds = me.gp.store;
		...
		me.upload_success(file,Ext.decode(serverData));// the call back function
	},
	
	upload_success : function (file,responseJson){},
	...
	onUploadError : function(file, errorCode, message) {//上传错误触发函数
		var me = this.customSettings.scope_handler;
		me.linkBtnEvent();
		var ds = me.gp.store;
		for(var i=0;i<ds.getCount();i++){
		...
		me.upload_failure(file,errorCode,message);
	},
	
	upload_failure : function (file,errorCode,responseJson){},
	
	onFileError : function(file,n){//校验文件大小
		switch(n){
	...
	onDiaogComplete : function(){ //当文件选取对话框关闭后触发的事件处理
		var me = this.customSettings.scope_handler;
		me.linkBtnEvent();
	},
	stopUpload : function() {//停止上传的函数
		if (this.swfupload) {
			this.swfupload.uploadStopped = true;
			this.swfupload.stopUpload();
		}
	},
	deleteAll : function(){//全部删除的函数
	...
	formatDelBtn : function(v){ //文件删除的函数
		return "<a href='javascript:void(0)' id='"+v+"'  style='color:blue' class='link-bttn' ext:qtip='移除该文件'>移除</a>";
	},
	linkBtnEvent : function(){
		Ext.select('a.link-bttn',false,this.gp.el.dom).on('click',function(o,e){
			var ds = this.gp.store;
			for(var i=0;i<ds.getCount();i++){
				var rec =ds.getAt(i);
				if(rec.get('id')==e.id){
					delUploadFile(rec.get('id'),rec.get('name'));//当它成功删除后才在ds中remove
					listName = listName.replace(rec.get('name'), "");
					ds.remove(rec);
				}
			}
...
Ext.reg('schemeUploadPanel',keel.schemeUploadPanel);
